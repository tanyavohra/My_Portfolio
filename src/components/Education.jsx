...
const Education = () => {
    ...
    return (<section id="education" className="py-24 relative overflow-hidden">
      {/* Background accents */}
      ...
      <div className="container mx-auto px-6" ref={ref}>
        ...
        <div className="grid lg:grid-cols-2 gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: -40 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <div className="relative">
              ...
              <div className="space-y-8">
                {education.map((item, index) => (<motion.div key={item.title} initial={{ opacity: 0, x: -20 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.4, delay: 0.3 + index * 0.1 }} className="relative pl-12">
                    {/* Timeline Dot */}
                    <div className={`absolute left-0 w-8 h-8 rounded-full flex items-center justify-center ${item.current
                ? "bg-primary"
                : item.achievement
                    ? "bg-gold"
                    : "bg-secondary"}`}>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6, delay: 0.3 }}>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              {achievements.map((achievement, index) => (<motion.div key={achievement.title} initial={{ opacity: 0, scale: 0.9 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.4, delay: 0.4 + index * 0.1 }} className={`glass rounded-2xl p-5 hover:border-primary/50 transition-colors relative overflow-hidden ${achievement.highlight ? "border-primary/30" : ""}`}>
                  {achievement.highlight && (<div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-primary/20 to-transparent"/>)}
            ...
            <div className="space-y-4">
              {activities.map((activity, index) => (<motion.div key={activity.title} initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.5 + index * 0.1 }} className="glass rounded-2xl p-5 hover:border-primary/50 transition-colors">
                  ...
                  {activity.venue && (<p className="text-xs text-muted-foreground mb-2">
                      📍 {activity.venue}
                    </p>)}
            ...
            {/* Hobbies */}
            <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.4, delay: 0.8 }} className="mt-8">
        ...
        ].map((hobby) => (<div key={hobby.name} className="glass rounded-xl p-4 text-center hover:border-primary/50 transition-colors">
